...
`;
  ...
  /**
   * A single, ready-to-run example method used when scaffolding a new project.
   * Folder: src/methods/example.user  ->  GET /example/user
   */
  exampleMethodInit() {
    return `'use strict';
...
/**
 * Metadata for the "example.user" method.
 * Route is generated automatically: GET /example/user
 */
class ExampleUserInitialize extends BaseInitialize {
  constructor() {
    super();
    this.initializer = {
      ...this.initializer,
  ...
  getResponses() {
    return {
      SUCCESS: { example: { message: 'Hello, World!' } },
    };
  }
}

module.exports = ExampleUserInitialize;
`;
  },
...
/**
 * Business logic for the "example.user" method.
 * Validated inputs are available on \`this\` (e.g. this.name).
 */
class ExampleUserAction extends BaseAction {
  async executeMethod() {
    const { name } = this;
...
module.exports = ExampleUserAction;
`;
  },
