...
const scaffold = {
  project(target, name, io) {
    ...
    // Library + a single example method + task.
    w('src/library/userlib/index.js', templates.library('user'));
    w('src/methods/example.user/init.js', templates.exampleMethodInit());
    w('src/methods/example.user/action.js', templates.exampleMethodAction());
  ...
  return JSON.stringify(
    {
      ...
      scripts: {
        start: 'rrf run',
        'start:ws': 'rrf run ws',
        'start:cron': 'rrf run cron',
        'start:all': 'rrf run all',
        postman: 'rrf postman:generate',
