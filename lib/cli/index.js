...
const commands = {
  ...
  create(args) {
    ...
    if (!installed) console.log(`  ${c.cyan((args.pm || 'npm') + ' install')}`);
    console.log(`  ${c.cyan('npm start')}        ${c.dim('# Express API  -> http://localhost:3000')}`);
    console.log(`  ${c.cyan('npm run start:ws')} ${c.dim('# WebSocket    -> ws://localhost:4000')}`);
    console.log('');
  },
...
function installDeps(target, pm) {
  ...
  console.log(c.dim(`\n  Installing dependencies with ${manager} ... (use --no-install to skip)`));
  ...
  console.log(
    c.yellow(
      `  Could not run "${manager} install" automatically. ` +
        `Run it manually inside the project.`
    )
  );
  return false;
}
