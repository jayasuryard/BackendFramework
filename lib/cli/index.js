...
const commands = {
  ...
  create(args) {
    const name = args._[0];
    if (!name) return fail('Usage: rrf create <app-name> [--no-install] [--pm npm|pnpm|yarn]');
    const target = path.resolve(process.cwd(), name);
    if (fs.existsSync(target) && fs.readdirSync(target).length) {
      return fail(`Directory "${name}" already exists and is not empty.`);
    }
    scaffold.project(target, name, { writeFile, ensureDir });
    console.log(c.bold(c.green(`\n✔ Project "${name}" created.`)));

    // Auto-install dependencies unless explicitly disabled.
    const skipInstall = args.install === false || args['no-install'] === true;
    const installed = skipInstall ? false : installDeps(target, args.pm);

    console.log('\n' + c.bold('Next steps:'));
    console.log(`  ${c.cyan(`cd ${name}`)}`);
    if (!installed) console.log(`  ${c.cyan('rrf install')}`);
    console.log(`  ${c.cyan('rrf run')}        ${c.dim('# Express API  -> http://localhost:3000')}`);
    console.log(`  ${c.cyan('rrf run ws')}     ${c.dim('# WebSocket    -> ws://localhost:4000')}`);
    console.log(`  ${c.cyan('rrf run cron')}   ${c.dim('# Scheduler')}`);
    console.log(`  ${c.cyan('rrf run all')}    ${c.dim('# Everything in one process')}`);
    console.log('');
  },

  /* --------------------------------------------------------------- install */
  install(args) {
    const ok = installDeps(process.cwd(), args.pm);
    if (ok) console.log(c.bold(c.green('\n✔ Dependencies installed.')));
    else fail('Dependency installation failed.');
  },
  ...
  async 'db:generate'() {
    ...
    if (!pool.isAvailable()) return fail("'pg' not installed. Run: rrf install");
    try {
  ...
  /* ------------------------------------------------------------------- run */
  run(args) {
    const target = (args._[0] || 'express').toLowerCase();
    switch (target) {
      case 'express':
      case 'http':
      case 'api':
      ...
      default:
        return fail(
          `Unknown run target "${target}". Use: rrf run [express|ws|cron|all] [--port <n>]`
        );
    }
  },
...
/**
 * Start every runtime (Express HTTP, WebSocket and the cron scheduler) inside a
 * single process. Used by `rrf run all`.
 */
function runAll(args) {
  require('../runtimes/express').start({ port: args.port });
  try {
    require('../runtimes/websocket').start({ port: args['ws-port'] });
  } catch (e) {
    console.log(c.yellow(`  WebSocket runtime skipped: ${e.message}`));
  }
  try {
    require('../runtimes/cron').start();
  } catch (e) {
    console.log(c.yellow(`  Scheduler runtime skipped: ${e.message}`));
  }
}

/**
 * Install dependencies in a freshly scaffolded project. Returns true on success.
 * Honors the chosen package manager (npm by default).
 */
function installDeps(target, pm) {
  const manager = pm || 'npm';
  console.log(c.dim(`\n  Installing dependencies ... (use --no-install to skip)`));
  ...
  console.log(
    c.yellow(
      `  Could not install dependencies automatically. ` +
        `Run "rrf install" again inside the project.`
    )
  );
  return false;
}
...
${c.bold('Project')}
  create <name>              Scaffold a new RRF project (auto-installs deps)
                             flags: --no-install, --pm npm|pnpm|yarn
...
${c.bold('Runtimes')}
  run [target] [--port n]    Run the app (default: express)
                             targets: express | ws | cron | all
